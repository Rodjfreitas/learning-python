...
while True:
    ...
    while jogos.isnumeric() == False:
       jogos = input(f'Inválido. Quantos jogos foram disputados por {dados["jogador"]}? ')
    ...
    while continuar not in 'SN':
       continuar = input('Inválido. Cadastrar novo jogador? [S/N]: ').strip().upper()
    if continuar == 'N':
       break
print(estatisticas)
print('-=' * 50)
...
for pos, valor in enumerate (estatisticas):
   print(f'{pos:>3} {valor["jogador"]:<20} {valor["jogos"]:<5} {valor["mediaGols"]:>5} {valor["totalGols"]:>5} {valor["gols"]}')
while True:
