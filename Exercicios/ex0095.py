...
for pos, valor in enumerate(estatisticas):
    print(
        f'{pos:>3} {valor["jogador"]:<20} {valor["jogos"]:<5} {valor["mediaGols"]:>5.1f} {valor["totalGols"]:>5} {valor["gols"]}')
while True:
    jogador = int(input('\nMostrar dados de qual jogador? '))
    if jogador != 999 and jogador < len(estatisticas):
        print(f'Levantamento do jogador {estatisticas[jogador]["jogador"]}')
        for i in range(0, len(estatisticas[jogador]["gols"])):
            print(f'  No jogo {i + 1} fez {estatisticas[jogador]["gols"][i]} gols.')
    elif jogador == 999:
        break
    else:
